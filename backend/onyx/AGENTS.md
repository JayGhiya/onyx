...
| Command | Config File | Confidence |
|---------|-------------|------------|
| `pip install -r backend/requirements/combined.txt` | `backend/requirements/combined.txt` | 0.92 |
| `pip install -r backend/requirements/dev.txt` | `backend/requirements/dev.txt` | 0.90 |

### Dev

| Command | Config File | Confidence |
|---------|-------------|------------|
| `uvicorn onyx.main:app --reload --port 8080` | `backend/onyx/main.py` | 0.95 |

### Test

| Command | Config File | Confidence |
|---------|-------------|------------|
| `pytest -xv backend/tests/unit` | `backend/pytest.ini` | 0.95 |
| `pytest backend/tests/integration` | `backend/pytest.ini` | 0.90 |

### Lint

| Command | Config File | Confidence |
|---------|-------------|------------|
| `pre-commit run --all-files` | `pyproject.toml` | 0.92 |
| `ruff check .` | `pyproject.toml` | 0.93 |

### Type Check

| Command | Config File | Confidence |
|---------|-------------|------------|
| `mypy .` | `pyproject.toml` | 0.95 |

## Dependency Guide

This project uses **pip** as its package manager (Python).

The full dependency catalog — including purpose and usage notes for each package — is maintained in [`dependencies_overview.md`](dependencies_overview.md).

No dependencies are currently catalogued. Add entries to `dependencies_overview.md` as new packages are introduced.

## Business Logic Domain

Onyx is an enterprise-grade AI-powered knowledge management and search platform. It ingests documents from a wide range of external sources — including Confluence, Google Drive, Slack, SharePoint, GitHub, Notion, Salesforce, and many others — and indexes them into a vector/keyword search backend (Vespa and OpenSearch) to enable semantically rich retrieval. The core user-facing experience is an LLM-backed chat interface where configurable AI "personas" (assistants) can answer questions by citing retrieved document chunks, calling tools (web search, image generation, code execution, MCP integrations), and optionally performing deep research across the knowledge base. The platform includes robust role-based access control, per-document permission syncing, multi-tenancy, federated connectors (e.g., real-time Slack search), knowledge graph extraction, and an agentic "Build" feature where users interact with a sandboxed coding agent to create web applications. Supporting infrastructure encompasses background indexing pipelines with Celery, token rate limiting, user memory/personalization, and extensive admin tooling for managing LLM providers, embedding models, OAuth flows, and connector credentials.

For a detailed mapping of data models, domain responsibilities, and key file references, see [`business_logic_references.md`](business_logic_references.md).

## App Interfaces

This section documents all inbound HTTP endpoints, outbound LLM inference calls, and internal scheduled tasks. For the full interface catalog, see [`app_interfaces.md`](app_interfaces.md).
...
| Line | Endpoint |
|------|----------|
| L1250 | `POST /logout` (per-backend) |
| L1288 | `POST /refresh` (per-backend) |
| L1685 | `GET /authorize` (OAuth) |
...
| L571 | `POST /oauth/callback` |
| L656 | `POST /user-credentials` |
| L984 | `GET /servers/persona/{assistant_id}` |
| L1013 | `GET /servers` |
...
| L154 | `POST /initiate` |
| L190 | `POST /callback` |
| L252 | `DELETE /{oauth_config_id}/token` |
...
| L190 | `PATCH /display-priorities` (admin agents) |
| L208 | `GET ""` (admin) |
| L223 | `GET ""` (admin agents) |
...
| File | Line | Call |
|------|------|------|
| `natural_language_processing/search_nlp_models.py` | L338 | `aembedding(model, input, timeout, api_key, api_base, api_version)` |

### Internal: Periodic Task Schedules (Celery)

| File | Line | Schedule |
|------|------|----------|
| `background/celery/tasks/beat_schedule.py` | L203 | `crontab(hour=0, minute=0, day_of_week=0)` — weekly Sunday midnight |